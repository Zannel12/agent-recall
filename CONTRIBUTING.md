...
9. Pull requests are validated by the `Tests / Python <version>` matrix and `Tests / Clean installation smoke` checks. They run under `pull_request` with read-only contents access; do not request `pull_request_target`, write permissions, secrets, publishing, or credential persistence for ordinary contribution checks.
