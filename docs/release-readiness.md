...
| PyPI publication | `BLOCKED` | `cited-vault-recall` / `0.2.0` was absent at the last registry check, but no owner-side non-secret publishing route is configured. | Owner configures a PyPI Trusted Publisher, then run a separately reviewed publication workflow. |
