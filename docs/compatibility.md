...
| CLI fallback | `agent-recall <vault> <query> [--format markdown|json]`, `agent-recall doctor`, `agent-recall reindex`, and `agent-recall-mcp --vault <vault>` are project entry points | Caller-selected local Markdown vault; derived index only at an explicit outside-vault destination | Search is read-only; doctor reports bounded local readiness without discovery; reindex replaces only explicit derived output | Local tests only | Repository regression suite |
