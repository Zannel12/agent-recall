...
The owner follows the official Claude Code MCP quickstart to add one local stdio server with the reviewed command:
...
Capture only non-sensitive evidence that Claude Code lists the bounded `search` tool and that a `privacy` query returns a citation with `relative_path: privacy.md`. Redact absolute paths. This is not proof that the host loaded Cited Vault Recall until the approved procedure succeeds and direct evidence is retained.

## Rollback

The owner removes only the created Claude Code MCP entry through the official host procedure, then verifies the entry is absent. Do not edit unrelated configuration.
